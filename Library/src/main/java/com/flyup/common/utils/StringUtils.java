...
public class StringUtils {
     ...
     * 判断字符串是否有值，如果为null或者是空字符串或者只有空格或者为"null"字符串，则返回true，否则则返回false
     */
    public static boolean isUserEmpty(String value) {

        if (value != null && !"".equalsIgnoreCase(value.trim()) && !"null".equalsIgnoreCase(value.trim()) && !"0".equalsIgnoreCase(value.trim())) {
            return false;
        } else {
            return true;
        }
    ...
    public static boolean isUrl(String str) {
        if (!isEmpty(str) && (str.startsWith("http") || str.startsWith("www"))) {
            return true;
        }
        return false;
    }
     ...
     * @return
     */
    public static String emojiFilte(String source) {
        ...
        String replaceResult = matcher.replaceAll("^_^");
